...
class Decoder(object):
    ...
    def __init__(self, isTraining, attribs):
        ...
        Args:
            ...
            attribs: A dictionary of attributes used by encoder like:
                ...
                target_vocab_size: Vocabulary size of output symbols
                embedding_size: Embedding size used to feed in input symbols
                out_prob (Optional): (1 - Dropout probability)
                sampling_probability (Optional): Sampling probability for sampling output
        ...
        if self.isTraining:
            self.out_prob = attribs['output_keep_prob']
            self.isSampling = False
            if ("sampling_probability" in attribs) and attribs["sampling_probability"] > 0.0:
                self.isSampling = True
                self.sampling_probability = attribs['sampling_probability']

        self.hidden_size = attribs['hidden_size']
        self.num_layers = attribs['num_layers']
        self.vocab_size = attribs['target_vocab_size']
    ...
    def set_cell_config(self):
        ...
        if self.num_layers > 1:
            ...
            cell = rnn_cell.MultiRNNCell([cell for _ in range(self.num_layers)],
                                         state_is_tuple=True)
